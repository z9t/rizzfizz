...
export async function exportDesignMd(options: DesignMdExportOptions): Promise<string[]> {
  ...
  // Try to read scrubbed prose
  let scrubbedProse = "";
  try {
    scrubbedProse = await readFile(join(options.input, "scrubbed", "DESIGN-neutral.md"), "utf-8");
    // Strip heading lines, keep body
    scrubbedProse = scrubbedProse
      .replace(/^#.*$/gm, "")
  ...
  const neutralMd = emitDesignMd(run, {
    name: options.name || `${run.source || "Design"} Neutral`,
